/** This class does everything relating to the creation and displaying of the grid.
It also modifies the grid when the player moves the tiles.
*/
...
public class Grid {
    ...
    public void setEmptyRow(int row) { //method which sets the row which the empty space is located
        if (row >= 0) {
            emptyRow = row;
        }
    }
    
    public int getEmptyRow() { //method which returns the row location of the empty space
        return emptyRow;
    }

    public void setEmptyColumn(int column) { //method which sets the column which the empty space is located
        if (column >= 0) {
            emptyColumn = column;
        }
    }
    
    public int getEmptyColumn() { //method which returns the column location of the empty space
        return emptyColumn;
    }
    
    private int size;
    
    public void setSize(int newSize) { //sets the grid size
        if (newSize > 0) {
            size = newSize;
        }
    }
    
    public int getSize() { //returns the grid size
        return size;
    }
    
    public String[][] grid;
    
	static String[][] easyConfig = { //sets the grid to have a 3x3 configuration
		{" "," "," "},
		{" "," "," "},
		{" "," "," "}
	};

	static String[][] mediumConfig = { //sets the grid to have a 4x4 configuration
		{" "," "," "," "},
		{" "," "," "," "},
		{" "," "," "," "},
        {" "," "," "," "}
    ...
    static String[][] hardConfig = { //sets the grid to have a 5x5 configuration
        {" "," "," "," "," "},
        {" "," "," "," "," "},
        {" "," "," "," "," "},
        {" "," "," "," "," "},
    ...
    */
    ...
    //This method converts the created ArrayList<Integer> into type String[][] which can be used to display the grid
